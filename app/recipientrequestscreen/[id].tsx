...
import MarkDonationAsReceived from "@/components/RecipientScreens/MarkDonationAsComplete";
import CancelBloodRequest from "@/components/RecipientScreens/CancelBloodRequest";
...
const SingleRequest = () => {
 ...
 const [status, setStatus] = useState<string>("pending")
  ...
  const id = 3;
  const backgroundColor = stateColors[status] || "#FFFFFF";
  return (
      ...
      <View className="flex flex-1 justify-between pl-2 pr-2">
        ...
        >
          <View className="flex flex-row items-center justify-between pt-2 pb-4 border-b-[4px] border-b-[#E8EAED]">
            ...
            <Text className="font-euclid text-[15px] font-[400] leading-[20.25px] text-[#4C4C4C]  ">
              <Text className="font-euclidBold text-[#0E0E0E] space-x-2 ">
                You made a blood request for Mary Onyekannmi
              </Text>
            </Text>
          </View>
          ...
          <View className="flex flex-row items-center justify-between p-4 border-b-[4px] border-b-[#E8EAED]">
            ...
            <View
              style={{ padding: 12, borderRadius: 16, backgroundColor }}
              className="p-3 rounded-2xl"
            >
              <Text className="font-euclidBold">{status}</Text>
            </View>
          </View>
        </View> 

        <View className="flex  flex-row justify-between gap-4 items-center p-4">
          <MarkDonationAsReceived setAcceptedRequest={setAcceptedRequest} />
          <CancelBloodRequest setRejectedRequest={setRejectedRequest} />
        </View>
      </View>
    </View>
  );
